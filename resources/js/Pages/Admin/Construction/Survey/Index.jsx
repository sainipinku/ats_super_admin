...
export default function SurveyIndex({ surveyPlans, surveySubmissions, projects, members }) {
    ...
    return (
        <ConstructionShell title="Survey Control" description="Create plans for assigned projects and close field submissions with clean reviews." variant="admin">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <StatCard label="Assigned Plans" value={stats.myPlans} />
            ...
            <div className="grid gap-6 xl:grid-cols-[430px,1fr]">
                <SectionCard title="Create Survey Plan" description="Schedule survey work for your assigned projects and attach the right team.">
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            planForm.post(route("admin.construction.survey.plans.store"), { preserveScroll: true });
                        }}
                        className="space-y-4"
                    >
                ...
                <div className="space-y-6">
                    <SectionCard title="Survey Plans" description="Plans and assigned members across your project scope.">
                        {surveyPlans.length ? (
                            ...
                            <EmptyState title="No survey plans yet." description="Start by creating a plan for one of your assigned projects." />
                        )}
                    </SectionCard>

                    <SectionCard title="Submission Reviews" description="Approve, reject, or send field work back for revision.">
                        {surveySubmissions.length ? (
                            ...
                            <EmptyState title="No submissions yet." description="Field submissions will appear here after surveyors complete their visits." />
                        )}
                    </SectionCard>
                </div>
            </div>
...
function SubmissionReviewCard({ submission }) {
    ...
    return (
        <form
            onSubmit={(e) => {
                e.preventDefault();
                form.post(route("admin.construction.survey.submissions.review", submission.id), { preserveScroll: true });
            }}
