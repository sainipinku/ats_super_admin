...
import {
  FiEye, FiEyeOff, FiEdit2, FiSave, FiX, FiUpload, FiTrash2,
  FiZoomIn, FiZoomOut, FiRotateCw, FiCheck
} from "react-icons/fi";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (croppedImage) {
      ...
      router.post(route('super.profile.photo.update'), formData, {
        onSuccess: () => {
  ...
  const handlePasswordChange = (e) => {
    ...
    router.post(route('super.profile.password.update'), formData, {
      onSuccess: () => {
  ...
  const handleRemoveProfilePhoto = () => {
    router.post(route('super.profile.photo.remove'), {}, {
      onSuccess: () => {
        setShowRemoveProfileDialog(false);
        // Clear the cropped image state if it exists
          ...
          <div className="p-[0px] md:p-[20px]">
            ...
            <div className="flex items-center justify-between flex-wrap md:flex-nowrap gap-[10px] md:gap-[25px] bg-[#5146E6] dark:bg-[#5146E6] px-[20px] py-[15px] rounded-[15px] shadow-md">
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-white">{UserDetails?.name}</h2>
                  <p className="text-white opacity-90">@{UserDetails?.username}</p>
                </div>
              </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
                <div className="w-full max-w-2xl bg-white dark:bg-[#080626] rounded-xl shadow-xl overflow-hidden mx-4">
                  <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 px-6 py-4">
                    <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Edit Profile Image</h3>
                    <button
                  ...
                  <div className="p-6">
                    <div className="flex flex-col items-center">
                      <div ref={containerRef} className="relative w-64 h-64 md:w-80 md:h-80 overflow-hidden border border-gray-300 rounded-lg mb-4 flex items-center justify-center">
                        <img
                          ref={imageRef}
                          src={selectedImage}
                          alt="Edit"
                ...
                <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      disabled={!isEditing}
                      className={`w-full px-4 py-2 rounded-lg border ${isEditing ? 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800' : 'border-transparent bg-gray-100 dark:bg-gray-700'} focus:ring-2 focus:ring-[#5146E6] focus:border-transparent`}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      disabled
                      className={`cursor-not-allowed w-full px-4 py-2 rounded-lg border ${isEditing ? 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800' : 'border-transparent bg-gray-100 dark:bg-gray-700'} focus:ring-2 focus:ring-[#5146E6] focus:border-transparent`}
                ...
                <form onSubmit={handlePasswordChange} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                      ...
                      {(passwordErrors.password_confirmation || passwordMatchError) && (
                        <span className="text-red-500 text-xs ml-2">
                          {passwordErrors.password_confirmation || passwordMatchError}
                        </span>
                      )}
