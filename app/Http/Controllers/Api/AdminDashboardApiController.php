...
class AdminDashboardApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $attendanceToday = AttendanceRecord::whereIn('project_id', $scopedProjectIds->all())
            ...
            ->latest()
            ->take(50)
            ->get(['id','project_id','member_id','attendance_type','check_in_at','check_out_at','hours_worked','attendance_date','status','gps_verified']);
        ...
        $pendingApprovals = [
            'survey_submissions' => SurveySubmission::with(['surveyVisit.surveyPlan.project'])
                ->whereIn('project_id', $scopedProjectIds->all())
                ->where('status', 'submitted')
                ->latest('submitted_at')
                ->take(10)
                ->get(),
            'drawing_approvals' => DrawingApproval::with(['revision.draftingJob.project'])
                ->whereIn('project_id', $scopedProjectIds->all())
                ->where('decision', 'pending')
                ->latest('requested_at')
                ->take(10)
                ->get(),
            'dpr_approvals' => DailyProgressReport::with(['project.company', 'submittedBy'])
                ->whereIn('project_id', $scopedProjectIds->all())
                ->where('status', 'submitted')
                ->latest('report_date')
                ->take(10)
                ->get(),
            'attendance_approvals' => AttendanceRecord::with(['project.company', 'member'])
                ->whereIn('project_id', $scopedProjectIds->all())
    ...
    private function buildStats($scopedProjectIds)
    {
        ...
        return [
            'total_projects' => Project::whereIn('id', $projectIdsArr)->count(),
            'active_projects' => Project::whereIn('id', $projectIdsArr)->where('status', 'active')->count(),
            'completed_projects' => Project::whereIn('id', $projectIdsArr)->where('status', 'completed')->count(),
            'draft_projects' => Project::whereIn('id', $projectIdsArr)->where('status', 'draft')->count(),
            'on_hold_projects' => Project::whereIn('id', $projectIdsArr)->where('status', 'on_hold')->count(),
            'companies_count' => Company::whereIn('id', Project::whereIn('id', $projectIdsArr)->pluck('company_id'))->count(),
            'clients_count' => Client::whereIn('id', Project::whereIn('id', $projectIdsArr)->pluck('client_id'))->count(),
            'team_members_count' => ProjectTeamMember::whereIn('project_id', $projectIdsArr)
                ->where('status', 'active')
                ->distinct('member_id')
                ->count('member_id'),
            'survey_plans' => SurveyPlan::whereIn('project_id', $projectIdsArr)->count(),
            'survey_pending_submissions' => SurveySubmission::whereIn('project_id', $projectIdsArr)
                ->where('status', 'submitted')
                ->count(),
            'drafting_queue' => DraftingJob::whereIn('project_id', $projectIdsArr)
                ->whereIn('status', ['queued', 'in_progress', 'submitted'])
                ->count(),
            'drawing_approvals_pending' => DrawingApproval::whereIn('project_id', $projectIdsArr)
                ->where('decision', 'pending')
                ->count(),
            'execution_plans' => ExecutionPlan::whereIn('project_id', $projectIdsArr)->count(),
            'execution_tasks' => ExecutionTask::whereIn('project_id', $projectIdsArr)->count(),
            'tasks_completed' => ExecutionTask::whereIn('project_id', $projectIdsArr)->where('status', 'completed')->count(),
            'tasks_in_progress' => ExecutionTask::whereIn('project_id', $projectIdsArr)->where('status', 'in_progress')->count(),
            'tasks_blocked' => ExecutionTask::whereIn('project_id', $projectIdsArr)->where('status', 'blocked')->count(),
            'dpr_pending_review' => DailyProgressReport::whereIn('project_id', $projectIdsArr)
                ->where('status', 'submitted')
                ->count(),
            'attendance_pending_review' => AttendanceRecord::whereIn('project_id', $projectIdsArr)
                ->where('status', 'pending')
                ->count(),
            'materials_count' => Material::whereIn('project_id', $projectIdsArr)->count(),
            'vehicles_count' => Vehicle::whereIn('project_id', $projectIdsArr)->count(),
            'equipment_count' => Equipment::whereIn('project_id', $projectIdsArr)->count(),
    ...
    private function buildTeamPerformance($scopedProjectIds)
    {
        ...
        $tasksDoneLast7 = ExecutionTaskAssignee::whereIn('project_id', $projectIdsArr)
            ->whereIn('execution_task_id', function ($q) {
                $q->select('id')->from('construction_execution_tasks')
    ...
    private function buildInventorySummary($scopedProjectIds)
    {
        ...
        $materials = DB::table('construction_materials')
            ->whereIn('project_id', $projectIdsArr)
            ->selectRaw('COUNT(*) as count')
            ->first();

        $lowStock = DB::table('construction_material_stocks')
            ->join('construction_materials', 'construction_materials.id', '=', 'construction_material_stocks.material_id')
            ->whereIn('construction_material_stocks.project_id', $projectIdsArr)
            ->whereColumn('construction_material_stocks.current_stock', '<=', 'construction_material_stocks.min_stock')
            ->count();

        $vehicles = DB::table('construction_vehicles')
            ->whereIn('project_id', $projectIdsArr)
            ->selectRaw('COUNT(*) as total, SUM(CASE WHEN status = "in_service" THEN 1 ELSE 0 END) as in_service, SUM(CASE WHEN status = "maintenance" THEN 1 ELSE 0 END) as maintenance, SUM(CASE WHEN status = "out_of_service" THEN 1 ELSE 0 END) as out_of_service')
            ->first();

        $equipment = DB::table('construction_equipment')
            ->whereIn('project_id', $projectIdsArr)
            ->selectRaw('COUNT(*) as total, SUM(CASE WHEN status = "in_service" THEN 1 ELSE 0 END) as in_service, SUM(CASE WHEN status = "maintenance" THEN 1 ELSE 0 END) as maintenance, SUM(CASE WHEN status = "out_of_service" THEN 1 ELSE 0 END) as out_of_service')
            ->first();

        $activeVehicleAssignments = VehicleAssignment::with(['vehicle', 'assignedTo', 'project:id,name'])
            ->whereIn('project_id', $projectIdsArr)
        ...
        $handovers = ProjectHandover::whereIn('project_id', $projectIdsArr)->count();

        return [
            'materials' => [
                'count' => (int) ($materials->count ?? 0),
